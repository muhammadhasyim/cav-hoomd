...
def load_temperature_energy_components(filename="potential_energy_components_vs_temperature.txt"):
    ...
    try:
        ...
        print(f"✓ Loaded temperature component data:")
...
def create_energy_temperature_interpolators(temp_data, temp_columns):
    """Create interpolating functions U(T) for energy components using extended fitting functions."""
    ...
    for display_name, col_name in energy_components.items():
        if col_name in temp_columns:
            print(f"Creating extended interpolator for {display_name} using column {col_name}")
            ...
            # Create extended fitting functions based on log-log scaling analysis
            if display_name == 'harmonic':
                # Extended harmonic: E = a*T/(1 + b*T)
                def extended_harmonic(x, a, b):
                    return a * x / (1 + b * x)
                
                try:
                    # Initial guesses for extended harmonic
                    a_harm_guess = np.abs(U_sorted).max() / T_sorted.max()  # Linear coefficient
                    b_harm_guess = 1e-3  # Small positive value
                    
                    harmonic_params, _ = curve_fit(
                        extended_harmonic,
                        T_sorted,
                        U_sorted,
                        p0=[a_harm_guess, b_harm_guess],
                        bounds=([0, 0], [np.inf, np.inf])
                    )
                    
                    # Create fitted function
                    def harmonic_U_of_T(T):
                        return extended_harmonic(T, *harmonic_params)
                    
                    # Create inverse function T(U) numerically
                    def harmonic_T_of_U(U):
                        # Solve U = a*T/(1 + b*T) for T
                        # U + U*b*T = a*T
                        # T = U / (a - U*b)
                        a, b = harmonic_params
                        return U / (a - U*b)
                    
                    print(f"  Extended Harmonic fit: a = {harmonic_params[0]:.6f}, b = {harmonic_params[1]:.6f}")
                    
                except Exception as e:
                    print(f"  Extended harmonic fitting failed: {e}, falling back to linear interpolation")
                    harmonic_U_of_T = interp1d(T_sorted, U_sorted, kind='linear', bounds_error=False, fill_value='extrapolate')
                    harmonic_T_of_U = interp1d(U_sorted, T_sorted, kind='linear', bounds_error=False, fill_value='extrapolate')
                
                U_of_T = harmonic_U_of_T
                T_of_U = harmonic_T_of_U
                
            else:
                # For LJ+Coulombic: Extended T^(3/5) scaling: E = E0 + A*T^(3/5)/(1 + C*T^(3/5))
                # Or simple T^(3/5) + B: E = A*T^(3/5) + B
                ...
                try:
                    ...
                    power_pred = power_law_with_intercept(T_sorted, *power_params)
                    residuals = U_sorted - power_pred
                    ss_res = np.sum(residuals**2)
                    ss_tot = np.sum((U_sorted - np.mean(U_sorted))**2)
                    power_r2 = 1 - (ss_res / ss_tot)
                    
                    print(f"  T^(3/5) + B fit: A = {a:.6f}, B = {b:.3f}, R² = {power_r2:.6f}")
                    
                    # Try extended T^(3/5) scaling
                    try:
                        e0_guess = U_sorted.min()
                        a_guess = a
                        c_guess = 1e-3
                        
                        extended_power_params, _ = curve_fit(
                            extended_power_law, 
                            T_sorted, 
                            U_sorted,
                        ...
                        # Calculate R-squared for extended fit
                        extended_power_pred = extended_power_law(T_sorted, *extended_power_params)
                        residuals_ext = U_sorted - extended_power_pred
                        extended_power_r2 = 1 - (np.sum(residuals_ext**2) / ss_tot)
                        
                        print(f"  Extended T^(3/5) fit: E0 = {extended_power_params[0]:.3f}, A = {extended_power_params[1]:.6f}, C = {extended_power_params[2]:.6f}, R² = {extended_power_r2:.6f}")
                        
                        # Use the better fit (higher R²)
                        if extended_power_r2 > power_r2:
                            print(f"  Using extended T^(3/5) fit (better R²)")
                            def power_U_of_T(T):
                                return extended_power_law(T, *extended_power_params)
                            
                            def power_T_of_U(U):
                                # Solve E = E0 + A*T^(3/5)/(1 + C*T^(3/5)) for T^(3/5)
                                # Let x = T^(3/5): U = E0 + A*x/(1 + C*x)
                                # (U - E0) * (1 + C*x) = A*x
                                # (U - E0) + (U - E0)*C*x - A*x = 0
                                # x*[(U - E0)*C - A] = -(U - E0)
                                # x = -(U - E0) / [(U - E0)*C - A]
                                # T^(3/5) = x, so T = x^(5/3)
                                e0, a, c = extended_power_params
                                denom = (U - e0) * c - a
                            ...
                            print(f"  Using simple T^(3/5) fit (better R²)")
                            def power_U_of_T(T):
                                return power_law_with_intercept(T, *power_params)
                            
                            def power_T_of_U(U):
                                # Solve U = A*T^(3/5) + B for T
                                # T^(3/5) = (U - B)/A
                                a, b = power_params
                                return ((U - b) / a)**(5/3)
                        ...
                        print(f"  Extended T^(3/5) fitting failed, using simple T^(3/5)")
                        def power_U_of_T(T):
                            return power_law_with_intercept(T, *power_params)
                    ...
                    print(f"  T^(3/5) fitting failed for {display_name}: {e}, falling back to linear interpolation")
            ...
            interpolators[display_name] = {
                ...
                'fit_type': 'extended' if display_name == 'harmonic' else 'power_law'
            }
        else:
    ...
    print(f"Successfully created {len(interpolators)} extended interpolators: {list(interpolators.keys())}")
...
def save_fictive_temperature_data(uniform_grid, fictive_temps, fictive_stds, 
                                 coupling_strength=None, output_dir='time_series_output'):
    """
    Save fictive temperature data matching their output format.
    ...
    uniform_grid : np.array
        Time grid
    fictive_temps : dict
        Dictionary of fictive temperature arrays by component
    fictive_stds : dict
        Dictionary of fictive temperature uncertainties
    coupling_strength : str, optional
        Coupling strength identifier (e.g., '1eneg03')
    output_dir : str
    ...
    # Determine coupling identifier from directory structure if not provided
    if coupling_strength is None:
        # Try to extract from current working directory path
        cwd = os.getcwd()
        if 'coupling_' in cwd:
            coupling_part = cwd.split('coupling_')[1].split('/')[0]
            coupling_strength = coupling_part.replace('eneg', 'epos0')
        else:
    ...
    # Create filename
    filename = f"coupling_{coupling_strength}_averaged_fictive_temperatures.txt"
    filepath = os.path.join(output_dir, filename)
    
    # Prepare data for output
    output_lines = []
    output_lines.append("# Averaged Fictive Temperatures from Energy Components")
    output_lines.append("# Generated using extended fitting functions from log-log scaling analysis")
    output_lines.append("# Columns:")
    
    # Create header line
    header_cols = ['time_ps']
    for comp_name in sorted(fictive_temps.keys()):
        header_cols.extend([f'fictive_T_{comp_name}_K'])
    
    output_lines.append("# " + "\t".join(header_cols))
    
    # Write data
    for i, time_point in enumerate(uniform_grid):
        data_row = [f"{time_point:.6f}"]
        
        for comp_name in sorted(fictive_temps.keys()):
            fictive_T = fictive_temps[comp_name][i]
            fictive_T_std = fictive_stds[comp_name][i] if comp_name in fictive_stds else np.nan
            
            if not np.isnan(fictive_T):
                data_row.append(f"{fictive_T:.6f}")
            else:
                data_row.append("NaN")
    ...
    print(f"✓ Fictive temperature data saved to: {filepath}")
    print(f"  Time range: {uniform_grid[0]:.3f} - {uniform_grid[-1]:.3f} ps")
    print(f"  Data points: {len(uniform_grid)}")
...
def main():
    ...
    parser.add_argument('--save-fictive', action='store_true',
                       help='Save fictive temperature data to time_series_output directory')
    parser.add_argument('--coupling-strength', type=str, default=None,
                       help='Coupling strength identifier for output filename (e.g., 1epos03)')
        ...
        if temp_data is not None:
            ...
            if interpolators:
                ...
                if fictive_temps:
                    ...
                    # Apply coupling strength extraction logic
                    coupling_strength = args.coupling_strength
                    if coupling_strength is None:
                        # Try to extract from directory path
                        directory_path = args.directory if hasattr(args, 'directory') and args.directory else os.getcwd()
                        if 'coupling_' in directory_path:
                            coupling_part = directory_path.split('coupling_')[1]
                            if '/' in coupling_part:
                                coupling_part = coupling_part.split('/')[0]
                            coupling_strength = coupling_part.replace('eneg', 'epos0')
                        else:
                            coupling_strength = 'unknown'
                        print(f"Detected coupling strength: {coupling_strength}")
    ...
    # Save fictive temperature data if requested
    if fictive_temps and args.save_fictive:
        print("Saving fictive temperature data...")
        fictive_files = save_fictive_temperature_data(uniform_grid, fictive_temps, fictive_stds, 
                                                     coupling_strength=coupling_strength)
        print(f"Fictive temperature data saved to: {', '.join(fictive_files)}")
