...
def main():
    ...
    parser.add_argument('--exp_dir', default=None, help='Experiment directory to process')
    parser.add_argument('--profile', default=None, help='Process all couplings from dataset profile')
    ...
    if args.profile:
        ...
        from dataset_profile import resolve_run_dir

        profile = setup_profile(args, default=args.profile)
        exit_code = 0
        for entry in profile.couplings:
            exp_dir = resolve_run_dir(profile, entry.id)
            logger.info(f"Profile batch: {entry.id} -> {exp_dir}")
            rc = _run_single_directory(
                exp_dir=exp_dir,
                args=args,
    ...
    if not args.exp_dir:
        logger.error("Provide --exp_dir or --profile")
        return 1

    exp_dir = Path(args.exp_dir).resolve()
    return _run_single_directory(exp_dir=exp_dir, args=args, start_time=start_time)


def _run_single_directory(exp_dir: Path, args, start_time: float) -> int:
    """Process and analyze F(k,t) for one experiment directory."""
