        ...
        if timestep - self.last_output_step >= self.output_period:
            ...
            if self.cavityforce is not None:
                ...
                try:
                    ...
                    cavity_dipole_self_energy = 0.0
                    cavity_total_potential_energy = 0.0
                    if timestep < 10:  # Only print for first few timesteps to avoid spam
            ...
            if self.molecular_langevin_method is not None:
                try:
                    ...
                    if timestep < 100 and timestep % 10 == 0:
                        print(f"Debug: timestep {timestep}, molecular_reservoir_energy = {molecular_reservoir_energy:.6f}")
                except AttributeError:
                    # molecular_reservoir_energy not available yet (before first simulation step)
                    molecular_reservoir_energy = 0.0
                    if timestep < 100 and timestep % 10 == 0:
                        print(f"Debug: timestep {timestep}, molecular reservoir energy not available yet")
            ...
            if self.cavity_langevin_method is not None:
                try:
                    ...
                    # Debug output for first few timesteps
                    if timestep < 100 and timestep % 10 == 0:
                        print(f"Debug: timestep {timestep}, cavity_reservoir_energy = {cavity_reservoir_energy:.6f}")
                except AttributeError:
                    # cavity_reservoir_energy not available yet (before first simulation step)
                    cavity_reservoir_energy = 0.0
                    if timestep < 100 and timestep % 10 == 0:
                        print(f"Debug: timestep {timestep}, cavity reservoir energy not available yet")
            ...
            if self.molecular_bussi_thermostat is not None:
                try:
                    ...
                    # Debug output for first few timesteps
                    if timestep < 100 and timestep % 10 == 0:
                        print(f"Debug: timestep {timestep}, molecular_bussi_total_reservoir_energy = {molecular_bussi_total_reservoir_energy:.6f}")
                    ...
                    molecular_bussi_rotational_energy = 0.0
                    if timestep < 100 and timestep % 10 == 0:
                        print(f"Debug: timestep {timestep}, molecular BussiReservoir energy not available yet")
            ...
            if self.cavity_bussi_thermostat is not None:
                try:
                    ...
                    # Debug output for first few timesteps
                    if timestep < 100 and timestep % 10 == 0:
                        print(f"Debug: timestep {timestep}, cavity_bussi_total_reservoir_energy = {cavity_bussi_total_reservoir_energy:.6f}")
                except (AttributeError, hoomd.error.DataAccessError):
                    # BussiReservoir energy not available yet (before first simulation step)
                    cavity_bussi_total_reservoir_energy = 0.0
                    cavity_bussi_translational_energy = 0.0
                    cavity_bussi_rotational_energy = 0.0
                    if timestep < 100 and timestep % 10 == 0:
                        print(f"Debug: timestep {timestep}, cavity BussiReservoir energy not available yet")
...
class DensityCorrelationTracker(hoomd.custom.Action):
    ...
    def act(self, timestep):
        ...
        current_rhok_real, current_rhok_imag = self.compute_rhok(positions)
...
class CavityMDSimulation:
    ...
    def setup_thermostat_parameters(self, dt):
        ...
        if self.molecular_thermostat.lower() == 'bussi':
            ...
            molecular_gamma = PhysicalConstants.gamma_from_tau_ps(self.molecular_thermostat_tau)
            molecular_method = hoomd.md.methods.Brownian(filter=molecular_filter, kT=kT, default_gamma=molecular_gamma)
        ...
        if self.incavity:
            ...
            if self.cavity_thermostat.lower() == 'langevin':
                ...
                if use_brownian:
                    ...
                    cavity_method = hoomd.md.methods.Brownian(filter=cavity_filter, kT=kT, default_gamma=cavity_gamma_brownian)
    ...
    def setup_trackers_and_loggers(self):
        ...
        self.status = Status(self.sim, PhysicalConstants.TIME_SECONDS * 1e9, self.time_tracker)
        ...
        if self.enable_fkt:
            ...
            self.density_corr_tracker = DensityCorrelationTracker(
                ...
                output_period=1000,  # Output summary every 1000 steps
                output_prefix=f'{self.name}-{self.replica}',
    ...
    def run_simulation(self):
        ...
        Runs for the specified runtime with proper timestep conversion.
        """
        # Use the pre-calculated total steps from calculate_physical_parameters
        ...
        self.log_info(f"Starting simulation for {self.runtime_ps:.1f} ps ({total_steps} steps)")
        self.log_info(f"Timestep: {actual_dt:.6f} a.u. ({actual_dt_ps * 1000:.3f} fs)")
        self.log_info(f"Effective steps per ps: {1.0/actual_dt_ps:.1f}")
