...
"""
Advanced Laser Drive Cavity Molecular Dynamics Experiment Runner

This script provides a comprehensive framework for running cavity MD simulations
with LASER DRIVE FORCING using the CavityMDSimulation class from the 
hoomd.cavitymd plugin. It replaces periodic coupling with laser drive forcing
F_L * cos(ω_L * t).

NEW LASER DRIVE FEATURES:
- Laser drive forcing: F_L * cos(ω_L * t) applied to cavity particle
- User-friendly frequency input in cm⁻¹ (automatically converted to a.u.)
- Customizable laser amplitude F_L in atomic units
- Full integration with existing analysis features
- Maintains all advanced simulation capabilities
...
BASIC USAGE:
   # Run a single experiment with laser drive (1560 cm⁻¹)
   python 18_laser_drive.py --molecular-bath bussi --cavity-bath langevin --coupling 1e-3 --laser --laser-frequency 1560 --laser-amplitude 1e-5 --runtime 1000
   
   # Run with CO₂ stretch frequency
   python 18_laser_drive.py --molecular-bath bussi --cavity-bath langevin --coupling 1e-3 --laser --laser-frequency 1560 --laser-amplitude 5e-6 --runtime 1000
   
   # Run with OH stretch frequency
   python 18_laser_drive.py --molecular-bath bussi --cavity-bath langevin --coupling 1e-3 --laser --laser-frequency 3400 --laser-amplitude 1e-5 --runtime 1000
   
   # Run without cavity (molecular-only simulation)
   python 18_laser_drive.py --molecular-bath bussi --no-cavity --runtime 1000
   
   # Run multiple replicas locally (replica 0 supported)
   python 18_laser_drive.py --molecular-bath bussi --cavity-bath langevin --coupling 1e-3 --laser --laser-frequency 1560 --laser-amplitude 1e-5 --replicas 0-4 --runtime 500
   
   # Run with finite-q cavity mode
   python 18_laser_drive.py --molecular-bath bussi --cavity-bath langevin --finite-q --coupling 1e-3 --laser --laser-frequency 1560 --laser-amplitude 1e-5 --runtime 1000

   # Standard constant coupling (fallback to regular behavior)
   python 18_laser_drive.py --molecular-bath bussi --cavity-bath langevin --coupling 1e-3 --runtime 1000
   
   # Run with fictive temperature feedback control
   python 18_laser_drive.py --molecular-bath bussi --cavity-bath langevin --coupling 1e-3 --runtime 1000 --enable-empirical-feedback --empirical-data-file equilibrium_data.txt

LASER DRIVE PARAMETERS:
- --laser: Enable laser drive forcing
- --laser-frequency: Laser frequency in cm⁻¹ (e.g., 1560 for CO₂ stretch)
- --laser-amplitude: Laser amplitude F_L in atomic units (e.g., 1e-5)
...
- --switch-time: Time-varying coupling activation (alternative to laser)
...
- --feedback-auto-stopping: Enable automatic coupling stop when fictive and harmonic temperatures converge
  NOTE: This stops the LASER DRIVE (sets amplitude to zero), NOT the feedback control.
...
- Organized directory structure with laser drive parameters in names
...
# =============================================================================
# SIMULATION FUNCTIONS (Using laser drive instead of periodic coupling)
# =============================================================================
...
def run_single_experiment(molecular_thermo, cavity_thermo, finite_q, 
                         ...
                         # NEW: Laser drive parameters
                         laser_enabled=False, laser_frequency_cm1=1560.0, laser_amplitude=1e-5, laser_start_time_ps=0.0,
                         # NEW: Mechanical cavity modulation parameters
                         mech_periodic=True, mech_frequency_cm1=100.0, mech_magnitude=1e-4,
    ...
    Extended with laser drive forcing and fictive temperature tracking support.
    ...
    try:
        ...
        if incavity:
            ...
            if laser_enabled:
                # Include laser parameters in directory name
                freq_str = f"_laser_{laser_frequency_cm1:.0f}cm1"
                amp_str = f"_amp_{laser_amplitude:.0e}".replace("-", "neg").replace("+", "pos")
                if laser_start_time_ps != 0.0:
                    start_str = f"_start_{laser_start_time_ps}ps"
                else:
                    start_str = ""
        ...
        if incavity:
            if laser_enabled:
                ...
                print(f"  Formula: F_laser = {laser_amplitude:.3e} * cos(2π × {laser_frequency_cm1:.1f} cm⁻¹ × t)")
            else:
        ...
        # Mechanical cavity modulation information
        if mech_periodic:
        ...
        # NOW INCLUDES: Laser drive parameters passed directly to CavityMDSimulation
        sim = CavityMDSimulation(
            ...
            # NEW: Laser drive parameters (NOW CONNECTED!)
...
def main():
    """Simplified main function for cavity MD experiments with laser drive."""
    parser = argparse.ArgumentParser(
        description='Advanced Laser Drive Cavity MD Experiment Runner',
    ...
    parser.add_argument('--laser', action='store_true',
                       help='Enable laser drive forcing: F_L * cos(ω_L * t)')
    parser.add_argument('--laser-frequency', type=float, default=1560.0,
                       help='Laser frequency in cm⁻¹ (default: 1560.0, CO₂ stretch)')
    parser.add_argument('--laser-amplitude', type=float, default=1e-5,
                       help='Laser amplitude F_L in atomic units (default: 1e-5)')
    parser.add_argument('--laser-start-time', type=float, default=0.0,
                       help='Time in ps to start laser drive (default: 0.0, immediate start)')
    ...
    parser.add_argument('--mech-magnitude', type=float, default=1e-6,
                       help='Mechanical modulation amplitude in atomic units (default: 1e-6)')
    
    # Existing time-varying coupling parameters (alternative to laser)
    ...
    print("Advanced Laser Drive Cavity MD Experiment Runner")
    print("="*60)
    
    # Validate laser drive parameters
    if args.laser and args.switch_time and not args.enable_empirical_feedback and not args.enable_simple_temp_drop:
        parser.error("Cannot use both --laser and --switch-time unless empirical feedback or simple temperature drop is enabled. "
    ...
    # Validate compatibility between laser and mechanical modulation
    ...
    if args.laser:
        ...
        print(f"Laser frequency (a.u.): {laser_freq_au:.6e}")
        print(f"Formula: F_laser = {args.laser_amplitude:.3e} * cos(2π × {args.laser_frequency:.1f} cm⁻¹ × t)")
    else:
        print(f"\n*** COUPLING CONSTANT: {args.coupling:.6e} a.u. ***")
    
    if args.mech_periodic:
        print(f"\n*** MECHANICAL CAVITY MODULATION ENABLED ***")
    ...
    if incavity:
        if args.laser:
            print(f"    Coupling type: LASER DRIVE")
            print(f"    Coupling strength: {args.coupling:.6e} a.u.")
            print(f"    Laser frequency: {args.laser_frequency:.1f} cm⁻¹")
            print(f"    Laser amplitude: {args.laser_amplitude:.6e} a.u.")
        else:
    ...
    for replica in replica_list:
        ...
        if success:
            ...
            if incavity:
                if args.laser:
                    print(f"SUCCESS: Replica {replica} completed successfully (laser drive: {args.laser_frequency:.1f} cm⁻¹, {args.laser_amplitude:.6e} a.u.)")
                else:
            ...
            if incavity:
                if args.laser:
                    print(f"ERROR: Replica {replica} failed (laser drive: {args.laser_frequency:.1f} cm⁻¹, {args.laser_amplitude:.6e} a.u.)")
                else:
    ...
    if incavity:
        if args.laser:
            print(f"Laser drive used: frequency {args.laser_frequency:.1f} cm⁻¹, amplitude {args.laser_amplitude:.6e} a.u.")
        else:
