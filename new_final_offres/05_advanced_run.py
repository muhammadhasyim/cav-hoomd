...
def run_single_experiment(molecular_thermo, cavity_thermo, finite_q, 
    ...
    try:
        ...
        if incavity:
            # For cavity simulations, include both coupling strength and frequency in directory name
            coupling_str = format_coupling_strength(coupling)
            freq_str = f"freq_{frequency:.1f}cm1"
            ...
            if switch_time_ps is not None:
                ...
                if decay_time_constant_ps is not None:
                    ...
                    exp_dir = Path(f"{coupling_str}_{freq_str}{switch_str}{decay_str}")
                else:
                    exp_dir = Path(f"{coupling_str}_{freq_str}{switch_str}")
            else:
                exp_dir = Path(f"{coupling_str}_{freq_str}")
        else:
            # For non-cavity simulations, include frequency for identification
            freq_str = f"freq_{frequency:.1f}cm1"
            exp_dir = Path(f"no_cavity_{freq_str}")
        ...
        print(f"  *** READING FRAME {frame} FROM {input_gsd_abs} ***")
        ...
        print(f"  ✓ Successfully initialized simulation from frame {frame}")
        print(f"  ✓ Input file: {input_gsd_abs}")
        print(f"  ✓ Replica {replica} ready to run...")
...
def main():
    ...
    for replica in replica_list:
        ...
        print(f">>> Will use FRAME {frame} from init-0.gsd for this replica <<<")
